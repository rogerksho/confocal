...
private:
	...
	int absolutePosition3;
	uint16_t lastPosition1;
	uint16_t lastPosition2;
	uint16_t lastPosition3;
...
public:
	...
	std::tuple<int, int, int> getThreeAbsoluteRotation();
	...
	std::tuple<double, double, double> getThreeAbsoluteRotationInDegrees();
	std::tuple<uint16_t, uint16_t, uint16_t> read_three(uint16_t registerAddress);
